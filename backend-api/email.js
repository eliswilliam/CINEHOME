...
// URLs configuráveis
const FRONTEND_URL = process.env.FRONTEND_URL || 'https://cinehome.onrender.com/reset.html';
const FRONTEND_LOGIN_URL = process.env.FRONTEND_LOGIN_URL || 'https://cinehome.onrender.com/profil.html';
...
router.get('/auth/google/login/callback', async (req, res) => {
  ...
  if (!code) {
    console.error('❌ Aucun code reçu de Google');
    return res.redirect(`https://cinehome.onrender.com/login.html?error=no_code`);
  }
  ...
  try {
    ...
    if (!accessToken) {
      ...
      return res.redirect(`https://cinehome.onrender.com/login.html?error=no_token`);
    }
    ...
    if (!email) {
      ...
      return res.redirect(`https://cinehome.onrender.com/login.html?error=no_email`);
    }
    ...
    if (!user) {
      ...
      return res.redirect(`https://cinehome.onrender.com/login.html?error=user_not_found&email=${encodeURIComponent(email)}`);
    }
    ...
    return res.redirect(`https://cinehome.onrender.com/login.html?error=oauth_failed`);
  }
});
